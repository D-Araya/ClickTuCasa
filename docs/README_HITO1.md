# 📎 Anexo histórico — Documentación original del Hito 1

> **Este documento se conserva sin modificaciones como registro histórico.**
> Corresponde a la documentación original de la **Unidad 1 — Fundamentos de Calidad y TDD en Java**, cuando el dominio vivía en el paquete `domain.model` y toda la orquestación pasaba por una única clase `RaffleService`.
>
> El código del repositorio **ya evolucionó** más allá de lo que describe este documento: en la **Unidad 3** el dominio fue refactorizado a Clean Architecture / DDD (paquetes `domain.entity`, `domain.valueobject`, `domain.repository` y una capa `application.usecase` con un caso de uso por flujo de negocio). Para la documentación **vigente** y actualizada del proyecto, ve al [`README.md`](./README.md) en la raíz del repositorio.
>
> Este anexo se mantiene únicamente con fines de trazabilidad académica (para poder comparar la evolución del diseño entre hitos) y **no debe usarse como referencia de la estructura de carpetas o clases actual**, que ya no coincide con lo descrito aquí (por ejemplo, `RaffleService`, `domain.model` y `RaffleServiceTest` ya no existen en el código fuente).
